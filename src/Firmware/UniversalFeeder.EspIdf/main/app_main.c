...
static void on_ip_address_changed(const char *ip_address)
{
    ...
    if (!s_mqtt_started && strcmp(ip_address, FEEDER_IP_ADDRESS_UNASSIGNED) != 0) {
        ESP_ERROR_CHECK(mqtt_service_start(s_device_id));
        s_mqtt_started = true;
    }
}
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(device_identity_get(s_device_id, sizeof(s_device_id)));
    ...
    ESP_LOGI(TAG, "Device: %s (%s)", DEVICE_NAME_PREFIX, s_device_id);
    ...
    ret = ble_provisioning_start(&credentials, ip_address, s_device_id, on_credentials_received);
    if (ret != ESP_OK) {
